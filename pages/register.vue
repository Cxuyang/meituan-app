  ...
  methods: {
    async sendMsg() {
      ...
      if (!namePass && !emailPass) {
        // 在nuxt.config.js中的module中引入了
        let {status, data} = await _this.$axios.post('/users/verify', { username: encodeURIComponent(_this.ruleForm.name), email: _this.ruleForm.email})
    ...
    register() {
      ...
      this.$refs['ruleForm'].validate(async (valid) => {
        if (valid) {
          let {status, data} = await _this.$axios.post('/users/signup', {
